            ...
            <div class="row justify-content-end mt-4">
                <div class="col-lg-10 pt-4 mt-4 text-right">
                    <h1 class="text-uppercase font-weight-light mt-4 pt-3 text-color-primary">
                        <?php
                            echo "Index Project ".$page_name." " . $halaman;
                        ?>
                    </h1>
                </div>
            </div>
        ...
        <div class="row" >
            <div class="col-md-12">
                <ul class="nav nav-pills sort-source mb-3 pb-2" data-sort-id="portfolio" data-option-key="filter" data-plugin-options="{'layoutMode': 'fitRows', 'filter': '*'}">
                    <li class="nav-item " data-option-value="*"><a class="nav-link " href="<?php echo base_url()?>en/project">All </a></li>
                    <?php
                    if(isset($nav_layanan)){
                        foreach($nav_layanan as $rowl){
                        ?>
                        <li class="nav-item <?php if($page_name==$rowl->judul_halaman) { echo "active"; }?>" ><a class="nav-link <?php if($page_name==$rowl->judul_halaman) { echo "active"; }?>" href="<?php echo base_url()?>en/project_categories/<?php echo  encrypt($rowl->halaman_id);?>"><?php echo $rowl->judul_halaman;?> </a></li>
                        <?php
                        }
                    }
                    ?>
